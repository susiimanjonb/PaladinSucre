  ...
  <div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      <div class="relative w-full sm:w-96">
        <input type="text" v-model="search" @input="debouncedFetch" placeholder="Buscar productos..." class="form-input pl-10 w-full">
        <svg class="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
      </div>
      <router-link :to="{ name: 'admin.products.create' }" class="btn-primary shrink-0">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="h-10 w-10 shrink-0 bg-gray-100 rounded-md overflow-hidden">
                    <img v-if="product.image_url" :src="product.image_url" class="h-10 w-10 object-cover">
                  </div>
                  <div class="ml-4">
                    <div class="text-sm font-medium text-gray-900">{{ product.name }}</div>
                    <div class="text-xs text-gray-500">{{ product.weight }}</div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <span :class="product.stock > 0 ? 'text-green-600 font-medium' : 'text-red-600 font-medium'">{{ product.stock }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <span v-if="product.is_featured" class="ml-1 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">★</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <router-link :to="{ name: 'admin.products.edit', params: { id: product.id } }" class="text-indigo-600 hover:text-indigo-900 mr-4">Editar</router-link>
                <button @click="confirmDelete(product)" class="text-red-600 hover:text-red-900">Eliminar</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <Modal :show="showDeleteModal" title="Confirmar Eliminación" @close="showDeleteModal = false">
      ...
      <template #footer>
        <button type="button" @click="showDeleteModal = false" class="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Cancelar</button>
        <button type="button" @click="handleDelete" :disabled="deleting" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:w-auto sm:text-sm">
...
import { getAdminProducts, deleteProduct } from '../../api/products.js';
...
const fetchProducts = async (page = 1) => {
    loading.value = true;
    try {
        const { data } = await getAdminProducts({ page, search: search.value });
